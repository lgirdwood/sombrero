...
struct data_ops {
	...
	void (*mult_subtract)(
		struct smbrr *dest, struct smbrr *a, struct smbrr *b,
	...
	void (*uchar_to_float)(
		struct smbrr *i,
		const unsigned char *c); /**< Unsigned char to float. */
	...
	void (*uint_to_uchar)(
		struct smbrr *i,
		unsigned char *c); /**< Unsigned int to unsigned char. */
};
...
#ifdef HAVE_OPENCL
extern const struct data_ops data_ops_1d_opencl;
extern const struct data_ops data_ops_2d_opencl;
extern const struct convolution_ops conv_ops_1d_opencl;
extern const struct convolution_ops conv_ops_2d_opencl;
#endif

...
/*! \fn struct smbrr *smbrr_new_from_area(struct smbrr *s,
        ...
        unsigned int y_end);
* \brief Create a new data element context from an area within an existing 2D
data
* element context.
* \ingroup data
*/
...
* \brief Create a new data element context from a section within an existing 1D
data
* element context.
* \ingroup data
*/
 ...
 * \brief Create a new data element context by copying an existing context and
 * data.
 * \ingroup data
 */
 ...
 * \brief Get the data stride for the original 2D data that was used to create
 * the elements within this data context.
 * \ingroup data
 */
...
/*! \fn void smbrr_significant_subtract(struct smbrr *a, struct smbrr *b,
        struct smbrr *c, struct smbrr *s);
 * \brief Add significant data elements from several contexts. A = B -
 (significant) C
 * \ingroup data
 */
...
/*! \fn void smbrr_mult_subtract(struct smbrr *dest, struct smbrr *a,
        struct smbrr *b, float c);
 * \brief Multiple data elements by scalar value and subtract from another
 context.
 *  A = A - B * value C
 * \ingroup data
 */
...
/*! \fn int smbrr_wavelet_significant_deconvolution(struct smbrr_wavelet *w,
        enum smbrr_conv conv, enum smbrr_wavelet_mask mask, enum smbrr_gain
 gain);
...
/*! \fn struct smbrr_object *smbrr_wavelet_get_object_at_posn(struct
smbrr_wavelet *w, int x, int y)
...
/*! \fn void smbrr_wavelet_set_ccd(struct smbrr_wavelet *w, float gain, float
bias, float readout)
